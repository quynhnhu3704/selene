...
test("Google callback preserves query, redirect and refresh cookie", async (t) => {
  const upstream = http.createServer((req, res) => {
    ...
    res.writeHead(302, {
      ...
      "Set-Cookie": "refreshToken=test; HttpOnly; Path=/",
    });
    res.end();
  });
  ...
  const result = await fetch(
    `${base}/google/callback?code=test-code&scope=email`,
    { redirect: "manual", headers: { Cookie: "previous=value" } },
  );
...
test("callback recovers if auth starts after connection refusal", async (t) => {
  ...
  const base = await gateway(t, `http://127.0.0.1:${port}`);
  const pending = fetch(`${base}/google/callback?code=test-code`, {
    redirect: "manual",
  });
...
test("unavailable auth returns login page instead of raw proxy error", async (t) => {
  ...
  const result = await fetch(`${base}/google/callback?code=test-code`, {
    redirect: "manual",
  });
  assert.equal(result.status, 302);
