...
export async function changeVariantStock(variantId, quantity) {
  for (let attempt = 0; attempt < 10; attempt += 1) {
    const { data: variant, error } = await supabase.from("product_variants")
      .select("stock_quantity").eq("variant_id", variantId).single();
    if (error) throw error;
    const stock = Number(variant.stock_quantity);
    if (!Number.isInteger(stock) || stock + quantity < 0) {
      throw new Error(`Biến thể ${variantId} đã hết hàng hoặc không đủ tồn kho.`);
    }
    const result = await supabase.from("product_variants")
      .update({ stock_quantity: stock + quantity })
      .eq("variant_id", variantId).eq("stock_quantity", stock)
      .select("variant_id").maybeSingle();
...
export async function restoreStock(items) {
  const results = await Promise.allSettled(items.map(item => changeVariantStock(item.variant_id, item.quantity)));
...
export async function reserveStock(items) {
  ...
  try {
    for (const item of items) {
      if (!Number.isInteger(item.quantity) || item.quantity <= 0) throw new Error("Số lượng sản phẩm không hợp lệ.");
