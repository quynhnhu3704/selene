...
export const PermissionModel = {
  getPermissionMatrix: async () => {
    const [permissions, roles] = await Promise.all([
      supabase.from("permissions").select("permission_id, name, description, status").order("name"),
      supabase.from("roles").select("role_id, name, status, role_permissions(permission_id)").order("role_id"),
    ]);
  ...
  setRolePermission: async (roleId, permissionId, enabled) => {
    ...
    const { error: insertError } = await supabase.from("role_permissions")
