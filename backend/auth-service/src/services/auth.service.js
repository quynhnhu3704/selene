...
export const forgotPasswordService = async (email) => {
  ...
  try {
    ...
    throw new Error(
      `Lỗi cập nhật mật khẩu vào Database: ${updateError.message}`,
    );
  }
  ...
  try {
    ...
    return {
      message:
        "Hệ thống đã cấp mật khẩu mới và gửi email khôi phục thành công!",
    };
  } catch (mailError) {
    console.error("Thực tế lỗi gửi Mail của Google:", mailError);
    throw new Error(
      "Cập nhật DB thành công nhưng server Mail bị từ chối gửi thư!",
    );
  }
};
...
export const loginWithGoogle = async (code) => {
  ...
  if (!account) {
    ...
    try {
      ...
      throw new Error(
        `Lỗi tạo hồ sơ người dùng từ Google: ${profError.message}`,
      );
    }
  }
...
export const refreshAccessToken = async (refreshToken) => {
  try {
    ...
    const dbTokenRecord = await RefreshTokenModel.findByAccountId(
      decoded.accountId,
    );
    if (!dbTokenRecord) {
    ...
    const isTokenMatch = await bcrypt.compare(
      refreshToken,
      dbTokenRecord.refresh_token_hash,
    );
    if (!isTokenMatch) {
    ...
    if (err.name === "TokenExpiredError") {
      throw new Error(
        "Phiên đăng nhập đã hết hạn từ lâu, vui lòng đăng nhập lại!",
      );
    }

    throw new Error(
      "Mã xác thực phiên làm việc không hợp lệ hoặc đã bị thay đổi!",
    );
  }
};
