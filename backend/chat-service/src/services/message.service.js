...
export const MessageService = {
  ...
  send: async (user, id, data) => {
    ...
    if (typeof content !== "string" || !content.trim() || content.trim().length > 2000 ||
      ...
      throw { status: 400, message: "Tin nhắn phải có từ 1 đến 2000 ký tự và mã gửi hợp lệ!" };
    }
  ...
  read: async (user, id, throughId) => {
    const conversation = await ConversationService.getAccessible(user, id);
    if (!/^\d+$/.test(String(throughId))) throw { status: 400, message: "Mã tin nhắn không hợp lệ!" };
    return MessageModel.markRead(id, conversation.customer_id, user.role === "customer", throughId);
  },
};
