...
export default function AdminSelect({
  ...
  const selected =
    options.find(
      (option) => String(option.props.value) === String(props.value),
    ) || options[0];
  ...
  const handleKeyDown = (event) => {
    ...
    if (["ArrowDown", "ArrowUp", "Home", "End"].includes(event.key)) {
      ...
      requestAnimationFrame(() => {
        const items = [
          ...rootRef.current.querySelectorAll('[role="option"]:not(:disabled)'),
        ];
  ...
  return (
    <div
      className={`dropdown adm-select ${className}`}
      ref={rootRef}
    ...
    >
      ...
      >
        ...
        <i
          className={`bi bi-caret-${open ? "up" : "down"}`}
          aria-hidden="true"
        />
      </button>
      {open && !props.disabled && (
        <ul
          ...
          className="adm-select-menu w-100 mt-1 shadow-sm"
        >
          {options.map((option) => (
