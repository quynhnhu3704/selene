...
export default function CustomerInfo({ customer, canViewOrder }) {
  return (
    <aside className="support-customer-info">
      ...
      <p>
        ...
        {customer?.phone_number || "Chưa có số điện thoại"}
      </p>
      <hr />
      <h6 className="fw-bold">Đơn hàng gần đây</h6>
      {customer?.orders_error && (
        <p className="text-warning mt-3" role="alert">
          {customer.orders_error}
        </p>
      )}
      ...
      {customer?.orders?.map((order) => (
        <div className="support-order" key={order.order_id}>
          {canViewOrder ? (
            <Link
              to={`/admin/don-hang/${order.order_id}`}
          ...
          <div className="mt-1">
            {Number(order.final_amount || 0).toLocaleString("vi-VN")}đ
          </div>
          <div className="mt-1">
