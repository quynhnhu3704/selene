    ...
    setConversations((prev) =>
      prev.map((item) =>
        item.conversation_id === selectedId
          ? { ...item, status: "open", assigned_staff_id: MOCK_USER.accountId }
          : item,
      ),
    );
  };
    ...
    <>
      <div className="d-flex align-items-center justify-content-between mb-4 flex-wrap gap-2">
        <div>
          ...
          <span className="text-muted">{waiting} hội thoại chờ xử lý</span>
        </div>
      ...
      <div className="support-workspace">
        <aside className="support-conversations">
          <div className="p-3 border-bottom">
            <label className="fw-semibold mb-2" htmlFor="support-filter">
              Hội thoại
            </label>
            <AdminSelect
              id="support-filter"
          ...
          <div className="support-conversation-scroll">
            {!filtered.length && (
              <p className="text-muted text-center p-3">Chưa có hội thoại.</p>
            )}
            {filtered.map((item) => (
              <button
              ...
              >
                <div className="d-flex justify-content-between gap-2">
                  <strong>{item.customer_name || "Khách hàng"}</strong>
                  {Number(item.staff_unread) > 0 && (
                    <span className="support-unread">{item.staff_unread}</span>
                  )}
                </div>
                <div className="support-preview">
                  {item.last_message || "Yêu cầu hỗ trợ mới"}
                </div>
          ...
          {selected ? (
            <>
              <div className="support-chat-header">
                <div>
                  <strong>
                    {selected.customer?.full_name || "Khách hàng"}
                  </strong>
                  <div className="mt-1">
                    <span className={`support-status ${selected.status}`}>
              ...
              {selected.status !== "closed" && !assignedToMe && (
                <div className="support-notice">
                  {selected.assigned_staff_id
                    ...
                    : "Nhận xử lý hội thoại để trả lời khách hàng."}
                </div>
              )}
              <ChatPanel
        ...
        {selected && (
          <CustomerInfo customer={selected.customer} canViewOrder={true} />
        )}
      </div>
    </>
  );
