...
export default function MessageInput({ disabled, onSend }) {
  ...
  const submit = async (event) => {
    ...
    if (pendingRef.current?.content !== content.trim()) {
      pendingRef.current = { content: content.trim(), client_id: crypto.randomUUID() };
    }
    try {
  ...
  return <form className="support-composer" onSubmit={submit}>
    <textarea aria-label="Nội dung tin nhắn" placeholder={disabled ? "Chưa thể gửi tin nhắn" : "Nhập tin nhắn của bạn..."}
      rows={2} maxLength={2000} value={content} disabled={disabled || sending}
      onChange={(event) => setContent(event.target.value)}
      onKeyDown={(event) => {
        if (event.key === "Enter" && !event.shiftKey && !event.nativeEvent.isComposing) submit(event);
      }} />
    <button className="btn btn-dark" type="submit" disabled={disabled || sending || !content.trim()} aria-label="Gửi tin nhắn">
